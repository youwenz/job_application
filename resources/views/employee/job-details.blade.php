...
{{--        @if (Gate::forUser(auth()->user())->check('isEmployee'))--}}
                <div class="flex flex-row gap-4">
                    ...
                    @endphp
                        ...
                        <!-- Bookmark Button -->
                    <form
                        action="{{ $isBookmarked ? route('jobListings.bookmark.remove', $jobListing->id) : route('jobListings.bookmark', $jobListing->id) }}"
                        method="POST">
                        @csrf
                        @if ($isBookmarked)
                            @method('DELETE')
                        ...
                        <button type="submit"
                                class="text-gray-500 hover:text-blue-500 transition duration-200 transform hover:scale-110">
                            @if ($isBookmarked)
                                <!-- Filled Bookmark -->
                            ...
                            @else
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                     stroke-width="1.5" stroke="currentColor" class="size-6 opacity-30">
