<x-public-layout>
    <form method="GET" action="{{ route('jobs.index') }}" class="bg-gray-100 shadow rounded w-[40%]">
        <div class="flex flex-wrap flex-col gap-4">
            <!-- Tags Filter -->
            <div class="relative">
                <label onclick="toggleFilter('tags')"
                       class="cursor-pointer font-bold px-4 py-4 hover:bg-blue-100 w-full block">Tags</label>
                <div id="tags" class="hidden space-y-1 p-2 rounded bg-blue-100 grid grid-cols-2 gap-x-6 gap-y-2">
                    @foreach ($tags as $tag)
            ...
            <div class="relative">
                ...
                <div id="states" class="hidden space-y-1 p-2 rounded bg-blue-100">
                    @foreach ($states as $state)
                        <div class="flex items-center gap-2 mb-4 pl-4">
                            <input type="checkbox" name="states[]" value="{{ $state }}" id="{{ $state }}"
                                   class="appearance-none h-4 w-4 border border-gray-300 rounded-2xl checked:bg-secondary checked:border-transparent focus:ring-2 focus:ring-blue-500 cursor-pointer transition duration-200"/>
                            <label for="{{ $state }}" class="text-gray-700 cursor-pointer">{{ $state }}</label>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="relative">
                ...
                <div id="remote" class="hidden space-y-1 p-2 rounded bg-blue-100">
                    <div class="flex items-center gap-2 mb-4 pl-4">
            ...
            <!-- Job Type Filter -->
            <div class="relative">
                <label onclick="toggleFilter('jobType')" class="cursor-pointer font-bold px-4 py-4 hover:bg-blue-100 block w-full">Job Type</label>
                <div id="jobType" class="hidden space-y-2 p-2 rounded bg-blue-100">
                    @foreach (['Full-Time', 'Part-Time', 'Contract', 'Temporary', 'Internship', 'Volunteer', 'On-Call'] as $type)
                        <div class="flex items-center gap-2 mb-4 pl-4">
                            <input type="checkbox" name="states[]" value="{{ $type }}" id="{{ $type }}"
                                   class="appearance-none h-4 w-4 border border-gray-300 rounded-2xl checked:bg-secondary checked:border-transparent focus:ring-2 focus:ring-blue-500 cursor-pointer transition duration-200"/>
                            <label for="{{ $type }}" class="text-gray-700 cursor-pointer">{{ $type }}</label>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="flex justify-center">
                <button type="submit" class="bg-secondary text-white py-2 px-10 rounded-lg mt-6 mb-6">Filter</button>
            </div>
        </div>
    </form>
